...
public class AppointmentsController(ISender mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> Get([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10,
        [FromQuery] string? searchTerm = null, [FromQuery] Guid? doctorId = null,
        [FromQuery] Guid? patientId = null, [FromQuery] AppointmentStatus? status = null,
        [FromQuery] DateTime? date = null, CancellationToken ct = default)
        => Ok(await mediator.Send(new GetAppointmentsQuery(pageNumber, pageSize, searchTerm, doctorId, patientId, status, date), ct));
    ...
    [Authorize(Roles = "Admin,Receptionist,Doctor")]
    public async Task<IActionResult> Cancel(Guid id, [FromBody] CancelAppointmentCommand cmd, CancellationToken ct)
