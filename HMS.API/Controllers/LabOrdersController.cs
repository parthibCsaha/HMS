...
public class LabOrdersController(ISender mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> Get([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10,
        [FromQuery] Guid? patientId = null, [FromQuery] LabTestStatus? status = null, CancellationToken ct = default)
    ...
    public async Task<IActionResult> Create([FromBody] CreateLabOrderCommand cmd, CancellationToken ct)
        => Ok(await mediator.Send(cmd, ct));

    [HttpPost("results")]
    [Authorize(Roles = "LabTechnician,Admin")]
    public async Task<IActionResult> AddResult([FromBody] AddLabResultCommand cmd, CancellationToken ct)
        => Ok(await mediator.Send(cmd, ct));
}
