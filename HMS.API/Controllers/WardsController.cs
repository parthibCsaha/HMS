...
public class WardsController(ISender mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateWardCommand cmd, CancellationToken ct)
        => Ok(await mediator.Send(cmd, ct));

    [HttpPut("{id:guid}")]
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateWardCommand cmd, CancellationToken ct)
