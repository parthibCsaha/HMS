...
public class GetVitalSignsQueryHandler(IVitalSignsRepository repo)
    : IRequestHandler<GetVitalSignsQuery, ApiResponse<IEnumerable<VitalSignsDto>>>
{
    public async Task<ApiResponse<IEnumerable<VitalSignsDto>>> Handle(
        GetVitalSignsQuery r,
        CancellationToken ct
    )
    {
        var items = await repo.GetByPatientAsync(r.PatientId, r.From, r.To, ct);
        var dtos = items.Select(v => new VitalSignsDto(
            v.Id,
            v.PatientId,
            v.RecordedAt,
