        ...
        IPatientRepository
{
    ...
    {
        var q = Context.Patients.Include(p => p.User).Where(p => !p.IsDeleted).AsNoTracking();
        ...
        q = query.SortBy?.ToLower() switch
        {
            ...
            "code" => query.IsDescending
                ? q.OrderByDescending(p => p.PatientCode)
                : q.OrderBy(p => p.PatientCode),
            "dateofbirth" => query.IsDescending
                ? q.OrderByDescending(p => p.DateOfBirth)
                : q.OrderBy(p => p.DateOfBirth),
            "city" => query.IsDescending
                ? q.OrderByDescending(p => p.City)
                : q.OrderBy(p => p.City),
