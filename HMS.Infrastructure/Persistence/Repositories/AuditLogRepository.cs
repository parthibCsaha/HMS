...
public class AuditLogRepository(AppDbContext context) : Repository<AuditLog>(context), IAuditLogRepository
{
    public async Task<(IEnumerable<AuditLog> Items, int TotalCount)> GetPagedAsync(
        PaginationQuery query, Guid? userId = null, string? action = null,
        string? entityName = null, DateTime? from = null, DateTime? to = null, CancellationToken ct = default)
    {
        ...
        if (from.HasValue) q = q.Where(a => a.LoggedAt >= from.Value);
        if (to.HasValue) q = q.Where(a => a.LoggedAt <= to.Value);
