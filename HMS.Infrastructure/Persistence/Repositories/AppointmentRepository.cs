...
public class AppointmentRepository(AppDbContext context) : Repository<Appointment>(context), IAppointmentRepository
{
    public async Task<(IEnumerable<Appointment> Items, int TotalCount)> GetPagedAsync(
        PaginationQuery query, Guid? doctorId = null, Guid? patientId = null,
        AppointmentStatus? status = null, DateTime? date = null, CancellationToken ct = default)
    {
        ...
        {
            "date" => query.IsDescending ? q.OrderByDescending(a => a.AppointmentDate) : q.OrderBy(a => a.AppointmentDate),
            "patient" => query.IsDescending ? q.OrderByDescending(a => a.Patient.User.FirstName) : q.OrderBy(a => a.Patient.User.FirstName),
            "doctor" => query.IsDescending ? q.OrderByDescending(a => a.Doctor.User.FirstName) : q.OrderBy(a => a.Doctor.User.FirstName),
            "status" => query.IsDescending ? q.OrderByDescending(a => a.Status) : q.OrderBy(a => a.Status),
            _ => q.OrderByDescending(a => a.AppointmentDate).ThenByDescending(a => a.AppointmentTime)
        };
    ...
    public async Task<IEnumerable<Appointment>> GetByDoctorAndDateAsync(Guid doctorId, DateTime date, CancellationToken ct = default)
    {
    ...
    public async Task<bool> HasConflictAsync(Guid doctorId, DateTime date, TimeSpan startTime, int durationMinutes, Guid? excludeId = null, CancellationToken ct = default)
    {
        ...
        return await q.AnyAsync(a =>
            ...
            a.AppointmentTime.Add(TimeSpan.FromMinutes(a.DurationMinutes)) > startTime, ct);
    }
}
