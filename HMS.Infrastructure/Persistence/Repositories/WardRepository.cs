...
{
    public async Task<(IEnumerable<Ward> Items, int TotalCount)> GetPagedAsync(
    ...
    )
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.SearchTerm))
        {
            ...
            q = q.Where(w =>
                w.Name.ToLower().Contains(search) || w.WardNumber.ToLower().Contains(search)
            );
        }
    ...
    public async Task<IEnumerable<Ward>> GetByDepartmentAsync(
        Guid departmentId,
        CancellationToken ct = default
    )
    {
        return await Context
            .Wards.Include(w => w.Department)
            .Where(w => w.DepartmentId == departmentId && !w.IsDeleted)
            .AsNoTracking()
            .ToListAsync(ct);
    }
}
