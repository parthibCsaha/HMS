...
public class DoctorRepository(AppDbContext context) : Repository<Doctor>(context), IDoctorRepository
{
    ...
    public async Task<(IEnumerable<Doctor> Items, int TotalCount)> GetPagedAsync(PaginationQuery query, CancellationToken ct = default)
    {
        var q = Context.Doctors
            .Include(d => d.User)
            .Include(d => d.Department)
            .Where(d => !d.IsDeleted)
            .AsNoTracking();
        ...
        {
            "name" => query.IsDescending ? q.OrderByDescending(d => d.User.FirstName) : q.OrderBy(d => d.User.FirstName),
            "specialization" => query.IsDescending ? q.OrderByDescending(d => d.Specialization) : q.OrderBy(d => d.Specialization),
            "department" => query.IsDescending ? q.OrderByDescending(d => d.Department.Name) : q.OrderBy(d => d.Department.Name),
            "experience" => query.IsDescending ? q.OrderByDescending(d => d.ExperienceYears) : q.OrderBy(d => d.ExperienceYears),
    ...
    public async Task<IEnumerable<Doctor>> GetByDepartmentAsync(Guid departmentId, CancellationToken ct = default)
    {
        return await Context.Doctors
            .Include(d => d.User)
