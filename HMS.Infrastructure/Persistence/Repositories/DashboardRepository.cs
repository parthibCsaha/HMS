...
public class DashboardRepository(IDapperContext dapperContext) : IDashboardRepository
{
    ...
    {
        using var connection = dapperContext.CreateConnection();
        return await connection.QueryAsync<OccupancyReportDto>(
            "SELECT * FROM sp_get_occupancy_report()"
        );
    }

    public async Task<IEnumerable<AppointmentTrendDto>> GetAppointmentTrendsAsync(
        DateTime from,
        DateTime to,
        CancellationToken ct = default
    )
    {
        ...
        return await connection.QueryAsync<AppointmentTrendDto>(
            "SELECT * FROM sp_get_appointment_trends(@From, @To)",
            new { From = from, To = to }
        );
    }

    public async Task<IEnumerable<RevenueSummaryDto>> GetRevenueReportAsync(
        DateTime from,
        DateTime to,
        CancellationToken ct = default
    )
    {
        ...
        return await connection.QueryAsync<RevenueSummaryDto>(
            "SELECT * FROM sp_get_revenue_report(@From, @To)",
            new { From = from, To = to }
        );
    }

    public async Task<PatientDemographicsDto> GetPatientDemographicsAsync(
        CancellationToken ct = default
    )
    {
        using var connection = dapperContext.CreateConnection();
        using var multi = await connection.QueryMultipleAsync(
            "SELECT * FROM sp_get_patient_demographics()"
        );
